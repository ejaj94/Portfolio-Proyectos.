// LearnCraft AI SaaS - Shared Frontend Logic in pt-PT
...
// Populate Dropdowns
async function loadUsersSelect(selectId, roleFilter = null) {
  ...
  try {
    ...
    select.innerHTML = `<option value="">Selecione o ${roleFilter || 'Utilizador'}...</option>`;
    users.forEach(u => {
      select.innerHTML += `<option value="${u.id}">${u.name} (${u.role}) - NIF: ${u.nif}</option>`;
    });
  } catch (err) {
...
async function loadCoursesSelect(selectId) {
  ...
  try {
    ...
    courses.forEach(c => {
      select.innerHTML += `<option value="${c.id}">${c.thumbnail_emoji} ${c.title} (${c.price.toFixed(2)} €)</option>`;
    });
  } catch (err) {
...
async function loadLessonsSelect(selectId) {
  const select = document.getElementById(selectId);
  if (!select) return;
  try {
    const res = await fetch('/api/lessons');
    const lessons = await res.json();
    select.innerHTML = '<option value="">Selecione a Lição...</option>';
    lessons.forEach(l => {
      select.innerHTML += `<option value="${l.id}">Mód.${l.module_number}: ${l.title} (${l.course_title})</option>`;
    });
  } catch (err) {
